...
If you tried to keep all your business knowledge in one long Claude conversation, you'd lose it. Compaction would summarize away the details. Restart the session, and you've lost everything that wasn't compressed.
...
- **`/hello`** runs at the start of a session. It reads the relevant files (README, CLAUDE.md, last session's notes, current state) and briefs you. The conversation now has fresh context, not yesterday's compressed summary.

- **`/goodbye`** runs at the end of a meaningful session. It writes the session's important takeaways to `SESSION-NOTES.md`. The next `/hello` will load those notes — even though the in-conversation memory will be gone.
...
- **Long sessions degrade.** If a conversation stretches past a few hours of real work, expect Claude to start losing earlier details. When you feel that happening, run `/goodbye` and start fresh with `/hello`.
- **Don't dump huge documents into the chat.** Save them to `raw/` and let `/ingest` process them into the wiki. The wiki page is small; the raw document is large. Claude reading the wiki page next session is much more efficient than re-reading the document.
...
- **`/brief` does the heavy lifting once a week.** It pulls everything together so the next session can start with the synthesized briefing instead of re-discovering connections.

## Cross-references

- [Three layers of memory](three-layers-of-memory.md) — why disk beats chat memory
- [The four commands](the-four-commands.md) — what each command actually writes
